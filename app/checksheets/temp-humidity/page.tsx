              ...
              return entries.map((floor) => (
                <section key={floor} aria-labelledby={`temp-floor-${floor}`}>
                  ...
                  <div className="flex flex-col divide-y divide-border border border-border">
                    {floors.get(floor)?.map((item) => <TargetListRow key={item.id} href={`/checksheets/temp-humidity/${item.id}`} name={item.name} floor={item.floor || item.name.match(/(\d+)층/)?.[0]} department={item.department} manager={item.manager} deleteTitle="이 항목을 삭제할까요?" deleteDescription={`${item.name} 항목과 입력된 온/습도 점검 내용이 함께 삭제됩니다. 이 작업은 되돌릴 수 없습니다.`} deleteAction={deleteTempHumidityTarget} id={item.id} />)}
                  </div>
                </section>
              ))
            })()}
