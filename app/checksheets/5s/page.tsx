            ...
            {[...groupByFloor(targetList)].map(([floor, items]) => items.length > 0 && (
              <FloorGroup key={floor} floor={floor}>
                <div className="flex flex-col divide-y divide-border border border-border">
                  {items.map((item) => (
                    <TargetListRow key={item.id} href={`/checksheets/5s/${item.id}`} name={item.name} department={item.department} manager={item.manager} deleteTitle="이 항목을 삭제할까요?" deleteDescription={`${item.name} 항목과 입력된 3정 5S 점검 내용이 함께 삭제됩니다. 이 작업은 되돌릴 수 없습니다.`} deleteAction={deleteFiveSTarget} id={item.id} />
                  ))}
                </div>
              </FloorGroup>
            ))}
